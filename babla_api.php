...
$titletext = '<a href="http://en.bab.la/dictionary/' . $from . '-' . $dest . '/' . $phrase . '">Babla Dictionary (' . tohtml($from) . "-" . tohtml($dest) . "):  &nbsp; <span class=\"red2\">" . tohtml($phrase) . "</span></a>";
...
if ($from != '' && $dest != '' && $phrase != '') {
	...
	$dic_url = 'http://en.bab.la/dictionary/' . urlencode($from) . '-' . urlencode($dest) . '/' . urlencode($phrase);
	$html = file_get_html($dic_url);
	$dic_data = array();
	// Find all translations
	foreach($html->find("div[class=result-block]") as $dic_table) {
		foreach($dic_table->find('div[class=result-row]') as $dic_row) {
			$dic_first_td_result = $dic_row->find('div[class=result-left]')[0]->find('a[class=result-link]');
			$dic_second_td_result = $dic_row->find('div[class=result-right]')[0]->find('a[class=result-link]');
			if($dic_first_td_result && $dic_second_td_result) {
				$dic_ps_result = $dic_row->find('div[class=result-right]')[0]->find('abbr');
				$dic_translation = array(
					"first" => $dic_first_td_result[0]->find('strong')[0]->plaintext,
					"second" => $dic_second_td_result[0]->plaintext,
					"ps" => $dic_ps_result[0]->plaintext
				);
				array_push($dic_data, $dic_translation);
			}		
		}
	}
	...
	if (count($dic_data) > 0) {
		...
		foreach ($dic_data as &$value) {
			...
			if ($word != '') {
				...
				echo '<span class="click" onclick="addTranslation(' . prepare_textdata_js($word) . "," . prepare_textdata_js($ps) . "," . prepare_textdata_js($origin) . ');"><img src="icn/tick-button.png" title="Copy" alt="Copy" /> &nbsp; ' . $word . " " . $ps_dot . '</span><br />' . "\n";
				$i++;
			}
		}
		echo "</p>";
		if ($i) {
		echo '<p>&nbsp;<br/>' . $i . ' translation' . ($i==1 ? '' : 's') . ' retrieved via <a href="http://en.bab.la" target="_blank">Babla API</a>.</p>';
		}
...
echo '&nbsp;<hr />&nbsp;<form action="babla_api.php" method="get">Unhappy?<br/>Change term: 
...
<input type="submit" value="Translate via Babla">
</form>';
